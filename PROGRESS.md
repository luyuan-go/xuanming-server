...
## 2026-09-15 Python 独立仓库拆分

- 补齐根目录 7 个 `Python*.cmd` 入口迁移：新仓库入口按 `<Go 工作区>/python/` 布局调用共享脚本，旧仓库移除重复入口并更新文档与停止命令提示。验证入口定位、参数透传、退出码和缺少共享工作区时的报错；未执行真实服务或集群启停。
- 按用户要求，将 `python/` 拆至 [lhl-go/XuanMing-Server](https://github.com/lhl-go/XuanMing-Server)，以原提交 `bd2be14b1c96e16ce047763eb06a30198386ffec` 为拆分基线，保留该目录的 24 次相关历史提交。
- 本次变更删除本仓库对 `python/` 的跟踪；共享协议、Go 对照实现、配置表、服务配置和部署工具继续留在本仓库。需要现有 Python 工具链时，按根 README 将独立仓库克隆到 `python/`，该目录由 `.gitignore` 忽略。
- 验证范围为 669 个原文件的 Git 树一致性及提交历史完整性。未执行服务启动、真实数据库测试或端到端验收；新仓库补充安装说明、共享工作区用法与原许可证。
- 发布顺序：先发布并核对独立仓库，再推送本仓库的目录移除提交。
...
## 2026-08-21 策划一键启动三支路并行准备

- **范围**：仅策划免 Docker fast 链；数据库模式/环境闸之后并行配置表生成、21 个唯一 Go target 的
  staging build、MySQL/Redis/Kafka/Envoy 启动。MySQL exact 协议 ready 后立即启动 migration，继续与
  其余基础设施重叠；准备分支全部成功后才发布和启动业务。K8s/部署目录零改动。
- **改表重启判定**：导表前后比较 `configtable/dist/manifest.json` SHA256；变化时只刷新当前真实引用
  `pkg/configtable` 的 8 个 runtime：player、battle_result、ds_allocator、inventory、dialogue、mission、
  matchmaker、matchmaker_pve。测试扫描非测试 Go wiring 与显式清单，防止新增消费者漏登记。当前是整批
  配置表变化到 8 个消费者的保守粒度，尚未做逐表映射。
- **竞态与失败边界**：生成 Go 文件先同目录完整 staging 再逐文件替换；导表后仅生成 Go/companion
  身份变化会作废投机 build，并在稳定输入上只重建一次；纯 JSON/manifest 数据变化只重启消费者，不
  重编 Go。prepared manifest 精确核对 `(target,runtime)` 无漏/无重后，发布前及 move 后逐项/整批复验
  长度/SHA256；stop/publish 失败恢复旧二进制和已停旧服务。applied receipt 绑定最终非 secret

...
|---|---|---|---|---|---|---|
| INC-20260824-003 | 2026-08-24 | P1 | near-miss / split-brain | **修法 C 已落码待部署**，D 待执行（未关闭） | **释放 owner 会抹掉再入屏障的判据**：为修 INC-20260824-001 缺口②而在首次 `ended` 心跳内、任何回收之前释放 owner，会把再入屏障从 `max(now,旧租约)+7s`（约 22s）降到 **0**——因为屏障的唯一判据（`owner_type=BATTLE` + `instance_uid`）正是 Release 的 UPDATE 抹掉的两列。边界②③ 对此零覆盖：它们管「删哪一条」，不管「此刻删安不安全」。**该洞是既有的**：登出释放对 owner 类型一视同仁，对局中登出早已同样抹掉屏障，本次只是把它扩大到每一局正常结算。今天仅 `mode=local` 可达（Agones+legacy 下 uid 恒空致释放静默 no-op；Model B 无该分支）。**已采纳 C 并落码**：把屏障从「归属指针的派生量」改成「玩家这一行的留存事实」——Release 释放 BATTLE 归属时按与 Begin 同一公式算出 `max(now,本实例租约截止)+skew` 盖进 `admit_not_before`，Begin 取 max 认回来（该列此前不在 Release 的 UPDATE 列清单里）。于是任何时刻释放都不会让围栏消失，边界①不再是调用方纪律，登出洞一并消失；两栈同构 + 各一条双变异击杀的回归用例。**未做**：D（把结算后大厅出生门从 ~22s 收敛到 ~7s，需 proto 加「实例已自行终结」信号）、§8 的「一次慢 RPC 甩掉半个花名册」（会让 001 原样复发）、`-race` 与真机 E2E | [事故报告](2026-08-24-p1-ds-allocator-ended-release-barrier-collapse.md) |
| INC-20260824-002 | 2026-08-24 | P0 | availability | 已修复待部署（未关闭） | **PVP 卡在「分配战斗服」**：`matchmaker-dev.yaml` 从未配 `ds_allocate_timeout`，回落代码默认 **60s**，而 `mode=local`+`launcher=editor` 的战斗 DS 冷启动远超 60s ⇒ `AllocateBattle` 每 60s 被客户端侧砍断并无限重分配。`ds_allocator` 侧预算 330s 从未先到期（有效预算 = min(客户端限额, 入站 deadline)）。`matchmaker-pve.yaml` 早已配 330s，这就是「PVE 能进、PVP 进不去」的分叉点。失败会**自愈式蒙对**（某次冷启动恰快于 60s 就成功），易被误判成抖动 | [事故报告](2026-08-24-p0-matchmaker-ds-allocate-timeout-fallback.md) |
| INC-20260824-001 | 2026-08-24 | P0 | availability | 根因确认；缺口①已落码待验证，缺口②修复方案复核阻断（未关闭） | **打完一局被踢回登录**：同一条「结算→回大厅」链上两个独立缺口同时成立。①主路径：Python 移植把 `inspect_battle_route` 改成单值返回+抛异常，两处调用点仍保留 Go 的双值解包，而 `BattleRouteState` 是 `IntEnum` 不可迭代 ⇒ 必抛 `TypeError` 且零日志（`IssueDSTicket` 恒 `code=1`、`err` 空），`TERMINAL→放行 Hub` 分支**从未执行过**。②降级路径：正常结算从不释放 owner——释放挂在需要**第二跳**心跳的 `errHeartbeatTerminal` 分支上，而 `state="ended"` 一局只发一次、周期心跳恒发 `"running"`、ACK 回调毫秒级 `StopBattleHeartbeat` ⇒ **第二跳物理上不可能到达**。两路同堵 ⇒ 客户端连续 3 次残缺 TARGET 后判弃回登录。仓内两条注释长期互相矛盾，**sweep 那条才对**。缺口②修法引入 INC-20260824-003，待定案 | [事故报告](2026-08-24-p0-python-settle-kick-to-login.md) |
| INC-20260821-001 | 2026-08-21 | P0 | crash / availability | 调查中（未关闭） | **Windows 本机 Kafka retention sharing violation 后退出**：删除 `pandora.player.presence-2` 过期 segment 时，`.timeindex` 重命名为 `.deleted` 被未知进程占用阻断；唯一 log dir 被 Kafka 判失败并关闭 broker。`9093/9094` 无 listener、Kafka JVM 0；未重启、未删数据、未完成 holder 定谳/恢复/故障注入/玩家 E2E | [事故报告](2026-08-21-p0-kafka-windows-retention-sharing-violation.md) |
